...
namespace cmsl
{
    ...
    namespace sema
    {
        ...
        private:
            ...
            using match_result_variant_t = std::variant<match_result::ok,
                                                        match_result::wrong_parameters_count,
            ...
            {
            public:
                explicit error_notes_reporter(errors::errors_observer& errs, const sema_function& function);
            ...
            match_result_variant_t params_match(const sema_function& function, const std::vector<std::reference_wrapper<const expression_node>>& call_parameters) const;

            std::vector<std::reference_wrapper<const expression_node>> to_vector_of_wrappers(const std::vector<std::unique_ptr<expression_node>>& params) const;
            std::vector<std::reference_wrapper<const expression_node>> to_vector_of_wrappers(const expression_node& param) const;

            void raise_error(const std::vector<function_match_result>& match_results) const;
