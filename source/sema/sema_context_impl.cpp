...
namespace cmsl::sema
{
    sema_context_impl::sema_context_impl(const sema_context* parent, context_type context_type)
            : m_parent{ parent }
    ...
    const sema_type* sema_context_impl::find_type(const ast::type_representation& name) const
    {
    ...
    const sema_type *sema_context_impl::find_referenced_type(const ast::type_representation &name) const
    {
    ...
    const sema_type* sema_context_impl::find_type_in_this_scope(const ast::type_representation& name) const
    {
    ...
    function_lookup_result_t sema_context_impl::find_function(const lexer::token& name) const
    {
    ...
    single_scope_function_lookup_result_t sema_context_impl::find_function_in_this_scope(const lexer::token& name) const
    {
    ...
    const sema_type* sema_context_impl::find_type_in_this_scope_with_predicate(Predicate&& pred) const
    {
        const auto found = std::find_if(std::cbegin(m_types), std::cend(m_types), pred);
        return found != std::cend(m_types) ? *found : nullptr;
    }

    const sema_type *sema_context_impl::find_reference_for(const sema_type& type) const
    {
    ...
    std::vector<std::reference_wrapper<const sema_type>> sema_context_impl::types() const
    {
