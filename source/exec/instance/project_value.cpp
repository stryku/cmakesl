...
std::vector<std::string> project_value::list_to_sources(
  facade::cmake_facade& cmake_facade, const list_value& sources)
{
  ...
  for (auto i = 0u; i < sources.size(); ++i) {
    ...
    const auto full_source_path = cmake_facade.current_directory() + '/' +
      source; // Todo: Handle already absolute paths.
