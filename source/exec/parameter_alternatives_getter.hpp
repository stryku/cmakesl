...
namespace details {
using alternatives_tuple_t = std::tuple<bool, int_t, double, std::string,
                                        inst::version_value, inst::list_value>;

template <inst::instance_value_alternative Alternative>
using alternative_type_t =
  decltype(std::get<Alternative>(std::declval<const alternatives_tuple_t>()));
...
private:
  ...
  static decltype(auto) get_impl(Params&& params, std::index_sequence<Is...>)
  {
    ...
    return std::make_tuple(
      details::get_param<alternatives[Is]>(*params[Is])...);
  }
};
}
