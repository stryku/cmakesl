...
TEST_F(OverloarResolutionTest,
       EmptyFunctionLookupResult_RaiseErrorAndReturnNull)
{
  errs_t errs;
...
TEST_F(OverloarResolutionTest,
       TriesInitializeReferenceWithTemporaryValue_RaiseErrorAndReturnNull)
{
  errs_t errs;
  const auto call_token = token_identifier("foo");
  StrictMock<sema_function_mock> function;
  ...
  function_signature signature{ function_name_token,
                                { parameter_declaration{
                                  function_param_type, param_name_token } } };
  EXPECT_CALL(function, signature()).WillRepeatedly(ReturnRef(signature));
...
TEST_F(OverloarResolutionTest, ParamsMatch_ReturnFunction)
{
  ...
  function_signature signature{ function_name_token,
                                { parameter_declaration{
                                  valid_type, param_name_token } } };
  EXPECT_CALL(function, signature()).WillOnce(ReturnRef(signature));
...
TEST_F(OverloarResolutionTest,
       GoodFunctionInUpperScopeHidByBadOne_RaiseErrorAndReturnNull)
{
  errs_t errs;
  const auto call_token = token_identifier("foo");
  ...
  single_scope_function_lookup_result_t scope_with_good_function{
    &good_function
  };
  single_scope_function_lookup_result_t scope_with_bad_function{
    &bad_function
  };
...
TEST_F(OverloarResolutionTest, GoodFunctionInUpperScope_ReturnFunction)
{
  ...
  single_scope_function_lookup_result_t scope_result{ &function };
  function_lookup_result_t lookup_result{
    single_scope_function_lookup_result_t{}, scope_result
  };
